...
## Current best (as of Round 4)

See `goldscalper/presets.py` -> `TREND_LOW_DRAWDOWN_*`. Full 6.5-year
dataset: 826 trades, 80.3% win rate, profit factor 1.66, Sharpe 2.18, max
drawdown -5.71%, +197.45% total return. Validated on a locked,
never-searched holdout year: 83.8% win rate, Sharpe 3.25, max DD -3.95%.

**Still not done:** no volatility/regime filter yet, no second instrument
for diversification, no forward paper-trading, no live execution bridge.
